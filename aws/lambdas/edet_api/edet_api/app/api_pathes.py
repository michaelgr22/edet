def mcknews(db, event):
    ...
    sql = """SELECT news_headline, news_date, news_category, news_imagelink, news_content FROM {}
    ...
    return list(map(lambda row: {'news_headline': row[0], 'news_date': row[1], 'news_category': row[2], 'news_imagelink': row[3], 'news_content': row[4]}, result))
...
def players(db, event):
    ...
    sql = """SELECT player_firstname, player_lastname, player_birthday, player_deployments, player_goals, player_position, player_imagelink
    FROM {0}
    ...
    return list(map(lambda row: {'player_firstname': row[0], 'player_lastname': row[1], 'player_birthday': row[2], 'player_deployments': row[3],
                                 'player_goals': row[4], 'player_position': row[5], 'player_imagelink': row[6]}, result))
...
def teammatches(db, event):
    ...
    sql = """SELECT match_date_time, home.team_showname as match_home_team_showname, away.team_showname as match_away_team_showname, 
...
ORDER BY match_date_time asc;""".format(matches_tablename, teams_tablename, leagues_tablename)
    ...
    return list(map(lambda row: {'match_date_time': row[0], 'match_home_team_showname': row[1], 'match_away_team_showname': row[2], 'match_link': row[3],
                                 'match_home_goals': row[4], 'match_away_goals': row[5], 'match_cancelled': row[6], 'match_league_showname': row[7]}, result))
...
def standings(db, event):
    ...
    sql = """SELECT standings_position, teams.team_showname as standings_team_showname, standings_games, standings_wins, 
...
ORDER BY standings_position asc;""".format(standings_tablename, teams_tablename)
    ...
    return list(map(lambda row: {'standings_position': row[0], 'standings_team_showname': row[1], 'standings_games': row[2], 'standings_wins': row[3],
                                 'standings_draws': row[4], 'standings_loses': row[5], 'standings_goals': row[6], 'standings_countered_goals': row[7],
                                 'standings_points': row[8]}, result))
...
def mainleague(db, event):
    ...
    sql = """SELECT league_showname, league_name, league_season 
FROM {0}
...
WHERE teams.team_name = %s AND teams.team_class = %s AND teams.team_season = %s;""".format(leagues_tablename, teams_tablename)
    ...
    return list(map(lambda row: {'league_showname': row[0], 'league_name': row[1], 'league_season': row[2]}, result))
...
ORDER BY match_date_time asc;""".format(matches_tablename, teams_tablename)
    ...
    return list(map(lambda row: {'match_date_time': row[0], 'match_home_team_showname': row[1], 'match_away_team_showname': row[2], 'match_link': row[3],
                                 'match_home_goals': row[4], 'match_away_goals': row[5], 'match_cancelled': row[6]}, result))
